    ...
    <div className="p-4 flex flex-col gap-2">
      <h2
        className={`text-lg font-bold line-clamp-1 ${
          theme === "light" ? "text-foreground" : "text-foreground-dark"
        }`}
      >
        {art.title}
      </h2>
...
const Gallery: React.FC<GalleryProps> = ({ showMoreCard = true, maxItems }) => {
  ...
  const [activeModal, setActiveModal] = useState<null | "gallery" | "buy">(
    null
  );
  ...
  const openGalleryModal = (art: Artwork) => {
    setSelectedImages([
      art.mainImageUrl,
      ...art.subImages.map((img) => img.imageUrl),
    ]);
  ...
  const shouldShowMoreCard =
    showMoreCard && artworks.length > displayedArtworks.length;
        ...
        {shouldShowMoreCard && (
          ...
          >
            ...
            >
              ...
              <h2 className="text-lg font-bold mb-2">Ver galería completa</h2>

              <p className="text-sm">Explora más cuadros en nuestra galería</p>
            </a>
          </article>
        )}
      </div>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto">
              {activeModal === "gallery" ? (
                ...
                selectedArtwork && (
                  <Prepago artwork={selectedArtwork} onClose={closeModal} />
                )
              )}
            </div>
          </div>
