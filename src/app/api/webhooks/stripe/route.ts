  ...
  try {
    ...
    switch (event.type) {
      ...
      case "charge.succeeded":
        ...
        console.warn(
          "⚠️ Received charge.succeeded event. Order creation is tied to payment_intent.succeeded."
        );
        break;

      case "charge.updated":
        console.warn(
          "⚠️ Received charge.updated event. Order creation is tied to payment_intent.succeeded."
        );
        break;
...
async function handlePaymentIntentSucceeded(
  ...
  try {
    ...
    const customer = parseMetadata(
      fullPaymentIntent.metadata.customer,
      "customer"
    );
    const address = parseMetadata(
      fullPaymentIntent.metadata.address,
      "address"
    );
    ...
    console.log(
      "🔍 Buscando productos en base de datos para enriquecer items..."
    );
    ...
    const enrichedItems = await Promise.all(
      items.map(async (item: any) => {
        ...
        if (!product) {
          console.warn(
            `🚫 Producto no encontrado: ID=${item.id}, Tipo=${item.type}`
          );
          return null;
        }

        console.log(
          `✅ Producto encontrado: ${product.title} | URL: ${product.mainImageUrl}`
        );
    ...
    if (validItems.length === 0) {
      console.error(
        "❌ No se encontró ningún producto válido en la base de datos"
      );
